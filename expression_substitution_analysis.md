...
The implementation uses preprocessor macros to support three different syntax modes:[Uploa# Analysis of Expression Substitution in tclParse.c
[expression_substitution_analysis.md](https://github.com/user-attachments/files/23112479/expression_substitution_analysis.md)# Analysis of Expression Substitution in tclParse.c
...
The calculation is documented in comments around lines 1148-1150:
```c
...
### Entry Condition (Lines 1480-1484)
...
### Initialization (Lines 1486-1494)
...
### Balanced Delimiter Scanning (Lines 1497-1524)
...
### Synthetic Command Generation (Lines 1543-1567)
...
### Token Transformation (Lines 1569-1578)
...
**Why repurpose instead of allocating a new token?**

The `$` substitution path always creates a VARIABLE token first. Rather than deleting it and creating a COMMAND token from scratch, the code simply repurposes the existing token by changing its type and contents. This is more efficient and avoids complications with token array management.
...
### Caller-Side Detection and Size Compensation (Lines 1133-1157)
...
The synthetic string is **leaked** because the code author doesn't yet know how to properly reclaim it. The fundamental issue is:

1. **The token points to the synthetic string** - The `varToken->start` pointer references the allocated synthetic command string
2. **Token information persists** - This token information gets saved somewhere in Tcl's internal structures (the exact location not yet discovered by the author)
3. **Points to saved script in memory** - Normally tokens point to the original saved script in memory, but here it points to dynamically allocated memory
4. **Unsafe to free prematurely** - The synthetic string can't be freed until the token is no longer valid, but determining when that occurs is unclear
5. **Possible solution involving compiler epoch** - The synthetic string might need to be linked to the compiler epoch, so if a re-parse occurs (e.g., from a repeating event), a new synthetic string would be created

**Impact of the leak:**
- **Compiled code (procs)**: Memory is NOT leaked for code in procedures, since procs are typically compiled once and reused
- **Repeated parsing contexts**: Memory IS leaked when the same code is parsed repeatedly:
  - `eval` commands that re-parse the same expression
  - Event handlers that get re-parsed on each trigger
  - `source` commands that re-read and re-parse files

**This is a known issue where the code author requires expert help from the Tcl core team.** The memory leak is understood but the proper cleanup mechanism within Tcl's complex token/parse lifecycle is not yet clear.
...
When syntax errors occur in the expression, the error messages currently **point to the synthetic command string, not the original source location**. This happens because:

1. **Token still points to synthetic** - The `varToken->start` points to the synthetic `[expr {...}]` string
2. **Error context uses token pointers** - When errors are reported, Tcl uses the token's start pointer to show context
3. **User sees wrong syntax** - Users see error messages referencing `[expr {...}]` rather than their original `$((expr))` syntax
4. **Harder to debug** - This makes debugging more difficult since the error doesn't point to what the user actually typed
...
The configurable mode system allows developers to choose the syntax that best fits their needs and avoids conflicts with existing code.
[expression_substitution_analysis.md](https://github.com/user-attachments/files/23112326/expression_substitution_analysis.md)
ding expression_substitution_an# Analysis of Expression Substitution in tclParse.c
